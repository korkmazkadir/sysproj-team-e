...
static void first_thread(void  *arg) {
    int port = (int)arg;
    const char *expectedMesssage = "message from rcv to snd";
    int connId = NetworkConnectAsClient(2, port);
    _ASSERT(connId >= 0);
...
static void second_thread(void *arg) {
    ...
    _ASSERT(connId >= 0);
    const char *stringToSend = "message from snd to rcv";
...
int main() {
    ...
    for (ii = 0; ii < CONNECTIONS; ++ii) {
        tids[ii * 2 + 0] = UserThreadCreate(&second_thread, (void *)(2 * ii));
        tids[ii * 2 + 1] = UserThreadCreate(&first_thread, (void *)(2 * ii + 1));
    }
    ...
    _ASSERT(NetworkConnectAsClient(2, 2 * CONNECTIONS) < 0);
